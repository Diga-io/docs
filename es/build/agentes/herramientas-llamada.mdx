...
4. Configura la herramienta. Puedes definir:
    - **Número de redirección**: El número al que se transferirá la llamada.
    - **Extensión de redirección**: Si usas un sistema PBX, puedes definir una extensión interna. En el caso de que no indiques número de redirección, la extensión se realizará con ese mismo número.
    - **Descripción de transferencia**: Explicación de cuándo el agente debe usar la herramienta
...
4. Configura la **condición** para definir cuándo el agente debe usar la herramienta
...
La herramienta de detección de contestador automático ayuda a tu agente a identificar si la llamada fue contestada por una máquina o por una persona real. Esto es especialmente útil para llamadas salientes.

### Cómo funciona

Es una herramienta que el **agente decide usar** cuando sospecha que una máquina contestó la llamada. No es una detección automática de contestador (AMD) que se ejecute en segundo plano.

Cuando el agente sospecha que una grabación contestó en lugar de una persona, usa esta herramienta. La herramienta ayuda al agente a confirmar si:
...
Según el resultado, el agente puede decidir cómo continuar (por ejemplo, dejar un mensaje breve y colgar, o continuar con el script normal).
...
<AccordionGroup>
  <Accordion title="Llamadas salientes automatizadas">
    En campañas de llamadas salientes, puedes instruir al agente para que solo continúe la conversación si confirma una persona real, o deje un mensaje grabado si es un contestador.

    **Ejemplo de configuración:**
    ```markdown
    Si sospechas que una máquina contestó en lugar de una persona:
    1. Usa la herramienta de detección de contestador
    2. Si confirmas una persona real: continúa con el script de ventas normal
    3. Si confirmas un contestador: deja un mensaje breve y cuelga
    ```
  </Accordion>
