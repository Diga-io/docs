...
| `channel` | Canal que usa la llamada |
| `contact` | Objeto de contacto (`id`, `identifier`, `contact_type`, `created_date`). El número de teléfono es `contact.identifier` |
| `agent_id` | Identificador del agente |
| `agent_version_id` | Identificador de la versión del agente |
| `phone_register_id` | Identificador del registro de teléfono |
| `start_time` | Fecha y hora de inicio |
| `created_date` | Cuándo se creó el registro de la llamada |
...
| `recording_url` | URL de la grabación de la llamada |
| `contact` | Objeto de contacto (`id`, `identifier`, `contact_type`, `created_date`). El número de teléfono es `contact.identifier` |
| `agent_id` | Identificador del agente |
| `agent_version_id` | Identificador de la versión del agente |
| `phone_register_id` | Identificador del registro de teléfono |
...
| **Modo de ejecución** | Si el agente **espera** la respuesta del flujo antes de continuar. **Síncrono** (por defecto): el agente se pausa y usa los datos devueltos. **Asíncrono**: el flujo se ejecuta en segundo plano y el agente continúa de inmediato sin esperar |
| **Audio de ejecución** | Lo que escucha el contacto mientras el agente espera en modo síncrono. **Ninguno**, **Tecleo** (sonidos de teclado) o **Música de espera** |
| **Pre-habla** | Si el agente dice un mensaje breve antes de ejecutar. **Auto** deja que el agente decida; **Forzar** hace que diga una frase corta sobre lo que va a hacer (por ejemplo, "Un momento, lo compruebo ahora mismo") justo antes de ejecutar el flujo, y luego espere en silencio. Nunca adelanta el resultado |
...
El agente extrae estos valores automáticamente del contexto de la conversación y los envía al flujo. Los tipos de dato de los parámetros incluyen **Texto**, **Número**, **Booleano** y **Array**.
