...
<Steps>
  ...
  <Step title="Define Parameters">
    ...
    **Body Parameters**: Sent in the request body (JSON)
    ...
    - **Data Type**: string, integer, number, boolean. Body parameters can additionally be **object** (with nested properties).
    ...
    - **Retrieval Mode**: How the agent gets this value:
      ...
      - Dynamic: value filled at runtime from a template/variable.
...
<Note>
  Tools get the same built-in handling as flows when the caller interrupts a running tool or the request fails mid-call: the execution is never cancelled, the agent is told the real outcome, and it never claims a result it can't confirm. See [When the caller interrupts a running flow](/en/build/flows/during-call-flows#when-the-caller-interrupts-a-running-flow).
</Note>
