...
| `channel` | Channel the call uses |
| `contact` | Contact object (`id`, `identifier`, `contact_type`, `created_date`). The phone number is `contact.identifier` |
| `agent_id` | Agent identifier |
| `agent_version_id` | Agent version identifier |
| `phone_register_id` | Phone register identifier |
| `start_time` | Start date and time |
| `created_date` | When the call record was created |
...
| **Execution mode** | Whether the agent **waits** for the workflow's response before continuing. **Synchronous** (default): the agent pauses and uses the returned data. **Asynchronous**: the workflow runs in the background and the agent continues immediately without waiting |
| **Execution audio** | What the caller hears while the agent waits in synchronous mode. **None**, **Typing** (keyboard sounds), or **Waiting music** |
| **Pre-speech** | Whether the agent says a short message before executing. **Auto** lets the agent decide; **Force** makes it say one short sentence about what it's about to do (for example, "Un momento, lo compruebo ahora mismo") right before running the flow, then wait in silence. It never announces the result in advance |
