...
When the call ends, the flow receives the complete call record — including the `status`, `duration`, `end_reason`, the contact's details (`contact.identifier` is the phone number), the call's `dynamic_variables`, and the full `messages` array (the transcript). See the [Call Finished trigger](/en/build/flows/diga-piece#call-finished) for the full field reference.
