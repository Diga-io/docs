...
On outbound calls, the agent can detect what answered the call and act accordingly:

- **Voicemail detection**: the agent detects when the call is answered by a voicemail and acts according to your configuration, leaving a custom message or hanging up directly.
- **Phone menu (IVR) detection**: the agent detects when the call is answered by an automated phone menu (IVR) and can navigate it to reach the right person or department.
