...
- **Voice-driven menus are not supported yet.** Systems that ask you to *say* the reason for your call instead of pressing keys can't be answered — the agent navigates with key presses only. These systems usually route to an operator after not hearing a response, where the agent picks the conversation up. If you need voice-driven menu support, [contact us](mailto:support@diga.io).
