...
In a single prompt agent, knowledge bases are assigned from the agent's **Customization** tab, within the **Knowledge Bases** section.

<Steps>
  <Step title="Open the Knowledge Bases section">
    Access the agent, go to the **Customization** tab and expand the **Knowledge Bases** section.
  </Step>
...
The knowledge base control links existing documents and bases; it does not create new content inline. To add new files, texts, or URLs, go to the [Knowledge bases](/en/build/knowledge-bases) section and create them there. They will then be available to link from this dropdown.

When no knowledge bases exist yet, the empty state links you directly to the Knowledge bases section so you can create your first one.
...
When you assign knowledge bases from the agent's **Customization** tab, they apply to **all nodes** in the conversational path. The agent will have access to this information at any point in the conversation.

The configuration is identical to a single prompt agent: expand the **Knowledge Bases** section, use the **Link Documents** dropdown to select documents.
...
If a node doesn't need the global knowledge bases, you can disable the **Inherit knowledge** toggle in the node's settings. When disabled:
...
<Note>
The **Inherit knowledge** toggle only appears in nodes when there is at least one knowledge base assigned at the agent's global level. If no global bases are configured, this control is not shown.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Use 'Context knowledge' for specific data">
