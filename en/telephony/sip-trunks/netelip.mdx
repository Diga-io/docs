...
<Steps>
  ...
  <Step title="Import numbers to Diga">
    ...
    Credentials are optional and only needed for outbound calls. To enable outbound calls, open the **Outgoing configuration** tab and provide the username and password that Netelip gives you in your [private panel](https://cloud.netelip.com/es/sipline/configurator/qr). You can find the Termination URI in this panel next to the *Server* label.

    Enter the Termination URI in the **Address** field **without** the `sip:` prefix (for example, `sip.netelip.com:5060`, not `sip:sip.netelip.com:5060`).
