...
<AccordionGroup>
  <Accordion title="Call lifecycle" icon="phone">
    ...
    | Answer Type Detected | On outbound calls with detection enabled: what answered the call (person, voicemail, phone menu) and the transcript of what was heard |
  ...
  <Accordion title="Agent responses" icon="bot">
    ...
    | Voicemail Message Left | The agent left your configured message on an answering machine, with the exact text it spoke |
    | Phone Menu Navigation Started | The agent started navigating a phone menu, with the goal or key sequence it pursues |
    | Phone Menu Option Selected | The agent pressed a key: includes the menu it heard, the key chosen and the reasoning behind the decision |
    | Phone Menu Navigation Finished | How the navigation ended (person reached, no answer, dead end) and the closing reasoning |
  </Accordion>
  ...
  <Accordion title="Call tools" icon="wrench">
    ...
    | Add Participant Tool Invoked | Adding a participant to the call was requested |
    | Add Participant Tool Completed | The participant was added successfully |
    | Add Participant Tool Failed | Adding the participant failed |
  </Accordion>
  ...
  <Accordion title="Knowledge base (RAG)" icon="book">
    Events about searches the agent runs against its knowledge base to retrieve relevant information during the call.

    | Event | Description |
    |-------|-------------|
    | RAG Search Completed | A knowledge base search returned relevant results |
    | RAG Search No Results | A knowledge base search returned no matching results |
    | RAG Search Failed | A knowledge base search failed |
  </Accordion>
  ...
  <Accordion title="Workflows / Flows" icon="diagram-project">
    Events about workflow (flow) executions tied to the call: deliveries to your endpoints, pre-call flows, and workflow tools invoked during the conversation.

    | Event | Description |
    |-------|-------------|
    | Workflow Queued | The workflow was queued for delivery |
    | Workflow Sent | The workflow HTTP request was sent |
    | Workflow Success | The workflow received a successful response |
    | Workflow Failed | The workflow received an error response |
    | Workflow Retry Exhausted | All workflow retries were exhausted |
    | Pre-Call Flow Triggered | A pre-call flow was triggered before the call started |
    | Pre-Call Flow Completed | A pre-call flow finished and returned its data |
    | Workflow Tool Invoked | A during-call workflow tool was invoked |
    | Workflow Tool Completed | A workflow tool finished successfully |
    | Workflow Tool Failed | A workflow tool failed |
    | Workflow Tool Interrupted | A workflow tool was interrupted |
    | Workflow Tool Triggered | A workflow tool was triggered |
    | Workflow Approval Granted | A workflow execution was approved |
  </Accordion>
</AccordionGroup>
